...
void Mapping(int **cross, int **road)
{
	...
	for (int speed_cnt = 0; speed_cnt < car_speed_cnt; speed_cnt++) //不同速度 
		for (int i = 0; i < road_txt_num; i++)  // 每条路径
		{
			//更新代价     //通行此路花费代价  长度 / 最高速 
...
void CopyRouteRoad(struct RouteRoad *p1, struct RouteRoad *p2) //将r2路径复制到r1上 注意P1->next要为NULL
{
...
void MAP_Floyd()
{
	...
	//ShowRouteRoad(MAP[SPEED_REAL_SUB(8)][CROSS_ORD_SUB(59)][CROSS_ORD_SUB(7)].Road);
}
